...
class host_memory_balance(object):
    ...
    def getFreeMemory(self, host):
        # getting free memory requires a REST call, so cache results
        if host.id not in self.free_memory_cache:
            try:
    ...
    def getBestVmForMigration(self, vms, maximum_vm_memory, memory_delta,
        ...
        # migrating a small vm is more likely to succeeded and puts less
    ...
    def do_balance(self, hosts_ids, args_map):
        ...
        minimum_host_memory = args_map.get('minimum_host_memoryMB',
